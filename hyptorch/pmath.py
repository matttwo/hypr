...
def tanh(x, clip_norm=15.0):   
    
    return torch.max(torch.min(x, clip_norm), -clip_norm).tanh()
...
class RiemannianGradient(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        scale = (1 - x.pow(2).sum(-1, keepdim=True)).pow(2) / 4
...
def _expmap0(u, c, clip_norm):
    ...
    gamma_1 = tanh(sqrt_c * u_norm, clip_norm) * u / (sqrt_c * u_norm)
